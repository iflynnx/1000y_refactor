...
type
    ...
    TWordString = record
        len: word;
        data: array[0..WORDSTRINGSIZE - 1] of char;
    end;
...
function getLeftStr(Str:string;posstr:string):string;
    ...
    begin
        ...
        begin
            ...
            begin
                if str[i] = '''' then
                begin
                    if (gstr <> '') and (Length(gstr) >= 2) then
                    begin
...
begin
    ...
    begin
        Result := '';
        Dest := Copy(str, cpos1 + Length(divider1), Length(str) - cpos1 - Length(divider1) + 1);
        exit;
    end;
...
begin
    ...
    buf[3] := _StrToInt(rdstr);
    str := GetValidStr3(str, rdstr, '.');
    buf[2] := _StrToInt(rdstr);
    str := GetValidStr3(str, rdstr, '.');
    buf[1] := _StrToInt(rdstr);
    str := GetValidStr3(str, rdstr, '.');
    buf[0] := _StrToInt(rdstr);
...
begin
    ...
    if (Length(str) = 10) then
    begin
        if word(str[1]) >= word('3') then str := ''
        else if (word(str[1]) = word('2')) and (word(str[2]) >= word('1')) then str := '';
    end;
    ...
    begin
        for i := 1 to Length(str) do
            if ((str[i] < char('0')) or (str[i] > char('9'))) and (str[i] <> '-') then
                flag := FALSE;
...
begin
    ...
    while TRUE do
    begin
        if LongInt(pb) >= LongInt(plast) then
            break;
        if pb^ = $0D then
        begin
            pb^ := 0;
    ...
    if LongInt(pstart) <= LongInt(plast) then
    begin
        str := StrPas(PCHAR(pstart));
        StringList.Add(str);
    end;
...
begin
    ...
    if (len >= WORDSTRINGSIZEALLOW) then len := WORDSTRINGSIZEALLOW;

    WordString.len := len;
    copymemory(@WordString.data[0], @str[1], WordString.len);
end;

function GetWordString(var WordString: TWordString): string;
begin
    Result := '';
    if (WordString.len <= 0) or (WordString.len > HIGH(WordString.DATA)) then EXIT;
    WordString.data[WordString.len] := #0;

    Result := StrPas(WordString.data);
end;
...
function SizeofWordString(var WordString: TWordString): Word;
begin
    Result := WordString.len + 2;
end;
{
function GetWordStringDataPointer(var WordString:TWordString):Pointer;
begin
    Result := @WordString.DATA[0];
end;

procedure SetWordStringDataSize(var WordString:TWordString; dsize:word);
begin
    WordString.len := dsize;
end;
}
{
procedure InverseWordString(var WordString:TWordString);
...
begin
    pret^ := char(0);
    if PWORD(@wstr)^ <= WSTRINGSIZEALLOW then
    begin
        wstr[PWORD(@wstr)^ + 2] := 0;
        move(wstr[2], pret^, PWORD(@wstr)^ + 1);                                // +1 篮 巩磊凯付瘤阜 0 鳖瘤 捞悼
    end
    else
    begin
    end;
...
procedure WScat(var dest, Sour: WString);
begin
    if (PWORD(@Dest)^ + PWORD(@Sour)^) <= WSTRINGSIZEALLOW then
    begin
...
begin
    ...
    if aInt >= 100000 then
        n := 100000;
    if aInt >= 1000000 then
        n := 1000000;
    if aInt >= 10000000 then
        n := 10000000;
    if aInt >= 100000000 then
        n := 100000000;
    if aInt >= 1000000000 then
        n := 1000000000;
...
begin
    ...
    begin
        ...
        for i := 0 to n - 1 - 1 do
        begin
            if wstr[2 + 1 + i] = byte('-') then
                exit;
            if (wstr[2 + 1 + i] < byte('0')) or (wstr[2 + 1 + i] > byte('9')) then
                exit;
    ...
    begin
        ...
        for i := 0 to n - 1 do
        begin
            if wstr[2 + i] = byte('-') then
                exit;
            if (wstr[2 + i] < byte('0')) or (wstr[2 + i] > byte('9')) then
                exit;
...
begin
    ...
    for j := 0 to n - 1 - slen + 1 do
    begin
        flag := TRUE;
        for i := 0 to slen - 1 do
            if wstr[2 + j + i] <> byte(astr[i + 1]) then
            begin
                flag := FALSE;
                break;
            end;
