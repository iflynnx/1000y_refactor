...
type
    ...
    public
        ...
        procedure LoadFromStreamCode(aStream:TStream);
...
var
    ...
    b, h, l         :byte;
begin
    for i := 0 to BYTESTRINGSIZE - 1 do
    begin
        b := ByteString[i];
        h := (b and $F0);
        l := (b and $0F);
...
begin
    ...
    begin
        if CompareText(FieldList[i], cField) = 0 then
        begin
            Result := i;
            exit;
        end;
...
begin
    ...
    for i := 0 to NameList.Count - 1 do
        if CompareStr(cName, NameList[i]) = 0 then
        begin
            Result := i;
            exit;
        end;
...
procedure TStringDb.LoadFromStreamCode(aStream:TStream);
var
    i, n, cnt       :integer;
    str, rdstr      :string;
...
begin
    ...
    for i := 0 to cnt - 1 do
    begin
        aStream.ReadBuffer(ByteString, sizeof(ByteString));
...
begin
    ...
    if (n mod BYTESTRINGSIZE) <> 0 then
    begin
...
begin
    ...
    _StrPCopy(ptemp, tempfieldstr + #13 + #10);
...
begin
    ...
    for i := 0 to DbStringList.Count - 1 do
    begin
        if Pos(NameList[i], cName) > 0 then
        begin
