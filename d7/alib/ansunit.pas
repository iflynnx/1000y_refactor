...
type
    ...
    功能：索引 算法
    说明：使用2分法搜索，支持字符字段，
          字符字段：关键字使用固定长度，比较浪费空间。如果使用位置搜索表也可实现。
    ...
    public
        ...
        // 2000.09.16 Insert Method 眠啊 by Lee.S.G
...
begin
    for i := 0 to 16 - 1 do
        IndexArr[i] := TAnsIndexclass.Create(aName, aDataSize, TRUE);
end;
...
begin
    ...
    if (FileHeader.rIdent <> FILERECORDDB_ID) then
    begin
        Stream.Free;
        exit;
    end;
    ...
    for i := 0 to FileHeader.rDataFieldCount - 1 do
    begin
        new(pfield);
    ...
    pb := allocpb;
    for i := 0 to OpenedHeader.rCount - 1 do
    begin
        GetMem(pdata, rsize);
        move(pb^, pdata^, rsize);
        inc(pb, rsize);
...
begin
    ...
    begin
        str := SList[j];
        str := GetValidStr3(str, rdstr, ',');
        if Length(rdstr) + 1 > tempnamesize then
            tempnamesize := Length(rdstr) + 1;
        for i := 0 to FieldList.Count - 1 do
        begin
            pf := FieldList[i];
            str := GetValidStr3(str, rdstr, ',');
            if Length(rdstr) + 1 > pf^.rFieldSize then
                pf^.rFieldSize := Length(rdstr) + 1;
        end;
    end;
...
begin
    Result := -1;
    if AnsIndex.Select(aname) = -1 then
        exit;
    if Length(aname) >= OpenedHeader.rNameSize then
        exit;
...
begin
    ...
    for i := 0 to FieldList.Count - 1 do
    begin
        if CompareText(PTFileRecordFieldData(FieldList[i]).rFieldName, fname) = 0 then
        begin
            Result := FieldList[i];
            exit;
        end;
...
begin
    ...
    for i := 0 to ACSFileHeader.Count - 1 do
    begin
        new(pd);
...
begin
    if str <> '' then
        DebugString.add(str);
    if DebugString.Count > 5000 then
        DebugString.delete(0);
    if Pos('Save', str) > 0 then
        SaveToFile;
end;
...
begin
    flag := TRUE;
    for i := 0 to DebugString.Count - 1 do
    begin
        if DebugString[i] = str then
            flag := FALSE;
    end;
    if flag then
        Add(str);
...
begin
    ...
    if (FileHeader.Ident <> KINGDBINDEX_ID) or (not FileHeader.boSavedIndex) then
    begin
        Stream.Free;
        exit;
    end;
...
begin
    ...
    begin
        ...
        for i := 0 to DataSize - 1 do
            //DataSize  每条记录 都固定长度 所以字符可以这样比较
        begin
...
begin
    ...
    for i := 0 to 32 - 1 do        //二分法最多32次，之后逐步搜索
    begin
...
begin
    ...
    if boString then
        StrPcopy(pchar(pid^.prdata), astr)
    else
        PINTEGER(pid^.prdata)^ := _StrToInt(astr);

    Result := FindIndex2(pid^.prdata);
    if Result <> DataList.Count then
        DataList.insert(Result, pid)
    else
        DataList.Add(pid);
end;
...
begin
    Result := -1;
    ai := DataSize;
    if (ai <= 0) or (ai >= 255) then ai := 255;
    FillChar(temp, ai, 0);
    if boString then
    begin
        if Length(aStr) > 255 then
            exit
        else
            StrPCopy(@temp, astr);
    end
...
begin
    ...
    for i := 0 to sbuffer - 1 do
        UnUsedList.Add(AllocFunction);
end;
...
var
    i               :integer;
begin
    for i := 0 to UnUsedList.Count - 1 do
        FreeFunction(UnUsedList[i]);
    for i := 0 to DataList.Count - 1 do
        FreeFunction(DataList[i]);
    UnUsedList.Free;
    DataList.Free;
    inherited destroy;
end;
...
var
    n               :Integer;
begin
    if index < DataList.Count then
    begin
        UnUsedList.Add(DataList[index]);
        DataList.Delete(index);

        if FMaxUnUsedCount < 0 then
            exit;

        if FMaxUnUsedCount < UnUsedList.Count then
        begin
            n := UnUsedList.Count;
