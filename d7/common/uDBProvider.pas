...
type
    ...
    private
        ...
        Header:TDBHeader;          //文件头结构
        RecordBuffer:TDBRecord;    //一个临时 空间

        IndexClass:TIndexClass;    //名字 索引
        BlankList:TList;           //空白 空间 列表
...
begin
    ...
    begin
        Result := 1;
    end else if pd1^.Name < pd2^.Name then
    begin
        Result := -1;
    end;
end;
...
begin
    ...
    Header.ID[3] := 0;
    if StrPas(@Header.ID) <> IndexID then
    begin
        Stream.Free;
        exit;
    end;
    ...
    try
        nCount := 0;
        for i := 0 to Header.IndexRecordCount - 1 do
        begin
        ...
        nCount := 0;
        for i := 0 to Header.BlankRecordCount - 1 do
        begin
...
begin
    ...
    while LowPos <= HighPos do
    begin
        pd := DataList.Items[MidPos];
        if pd^.Name = aName then
        begin
            Result := pd^.RecordNo;
            exit;
        end else if pd^.Name > aName then
        begin
            HighPos := MidPos - 1;
        end else
        begin
...
begin
    ...
    try
        ...
        begin
            ...
            if RecordBuffer.suser = 1 then
            begin
                nCode := IndexClass.Add((RecordBuffer.PrimaryKey), i);
                if nCode <> DB_OK then
                begin
                    //错误 增加 到索引 失败
                    ShowInfo(format('Invalid Data at %d (%s)', [i + 1, (RecordBuffer.PrimaryKey)]));
                    ShowInfo(format('-> %s', [IndexClass.GetLastErrorStr]));
                    ShowInfo(format('-> %s,%s', [(RecordBuffer.PrimaryKey), (RecordBuffer.MasterName)]));

                    nPos := IndexClass.Select((RecordBuffer.PrimaryKey));
                    if nPos >= 0 then
                    begin
...
begin
    ...
    try
        ...
        if (aDBRecord^.PrimaryKey) <> aIndexName then
        begin
            Result := DB_ERR_NOTFOUND;
            exit;
        end;
...
begin
    ...
    try
        ...
        if (DBRecord.PrimaryKey) <> aIndexName then
        begin
            Result := DB_ERR_INVALIDDATA;
            exit;
        end;
...
var
    ...
    RetStr          :string;
begin
    RetStr := (aDBRecord^.PrimaryKey);
    RetStr := RetStr + ',' + (aDBRecord^.MasterName);
    RetStr := RetStr + ',' + (aDBRecord^.Guild);
    RetStr := RetStr + ',' + (aDBRecord^.LastDate);
    RetStr := RetStr + ',' + (aDBRecord^.CreateDate);
    RetStr := RetStr + ',' + (aDBRecord^.Sex);
    ...
    RetStr := RetStr + ',' + IntToStr(aDBRecord^.CurHeadSeek);
    RetStr := RetStr + ',' + IntToStr(aDBRecord^.CurArmSeek);
    RetStr := RetStr + ',' + IntToStr(aDBRecord^.CurLegSeek);
    for i := 0 to 10 - 1 do
    begin
    ...
    for i := 0 to 8 - 1 do
    begin
        RetStr := RetStr + ',' + (aDBRecord^.WearItemArr[i].rName) + ':' + IntToStr(aDBRecord^.WearItemArr[i].rColor) + ':' + IntToStr(aDBRecord^.WearItemArr[i].rCount);
    end;
    for i := 0 to 30 - 1 do
    begin
        RetStr := RetStr + ',' + (aDBRecord^.HaveItemArr[i].rName) + ':' + IntToStr(aDBRecord^.HaveItemArr[i].rColor) + ':' + IntToStr(aDBRecord^.HaveItemArr[i].rCount);
    end;
    for i := 0 to 30 - 1 do
    begin
        RetStr := RetStr + ',' + (aDBRecord^.HaveMagicArr[i].Name) + ':' + IntToStr(aDBRecord^.HaveMagicArr[i].Skill);
    end;
...
begin
    ...
    (@DBRecord.PrimaryKey, uPrimaryKey);
    (@DBRecord.MasterName, uMasterName);
    (@DBRecord.Guild, uGuild);
    (@DBRecord.LastDate, uLastDate);
    (@DBRecord.CreateDate, uCreateDate);
    (@DBRecord.Sex, uSex);
    ...
    DBRecord.CurHeadSeek := _StrToInt(uCurHeadSeak);
    DBRecord.CurArmSeek := _StrToInt(uCurArmSeak);
    DBRecord.CurLegSeek := _StrToInt(uCurLegSeak);
    ...
    begin
        ...
        DBRecord.WearItemArr[i].rName := uname;
        DBRecord.WearItemArr[i].rColor := _StrToInt(ucolor);
        DBRecord.WearItemArr[i].rCount := _StrToInt(ucount);
    end;
    for i := 0 to 30 - 1 do
    begin
        rdstr := uHaveItem[i];
        ...
        DBRecord.HaveItemArr[i].rName := uname;
        DBRecord.HaveItemArr[i].rColor := _StrToInt(ucolor);
        DBRecord.HaveItemArr[i].rCount := _StrToInt(ucount);
    end;
    for i := 0 to 30 - 1 do
    begin
        rdstr := uHaveMagic[i];
        ...
        (@DBRecord.HaveMagicArr[i].Name, uname);
