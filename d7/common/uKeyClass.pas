...
type
    ...
    public
        ...
        function SelectByIndex(aindex:Integer):Pointer;
        function getDataList:TList;
    end;
...
{    TMultiStringKeyClass = class
    ...
    TStringKeyListClass = class(TStringKeyClass)
    public
        constructor Create;
        destructor Destroy; override;
    ...
    TIntegerKeyListClass = class(TIntegerKeyClass)
    public
        constructor Create;
        destructor Destroy; override;
...
begin
    ...
    if pd1^.StringKey > pd2^.StringKey then
    begin
        Result := 1;
    end else if pd1^.StringKey < pd2^.StringKey then
    begin
        Result := -1;
    end;
end;
...
begin
    ...
    if pd1^.IntegerKey > pd2^.IntegerKey then
    begin
        Result := 1;
    end else if pd1^.IntegerKey < pd2^.IntegerKey then
    begin
        Result := -1;
    end;
end;
...
begin
    ...
    begin
        ...
        if pd^.StringKey = aKey then
        begin
            Result := MidPos;
            exit;
        end else if pd^.StringKey > aKey then
        begin
            HighPos := MidPos - 1;
        end else
        begin
...
//    Result := false;
    ...
    if Select(aKey, nStartPos, nEndPos) > 0 then
    begin
        for i := nEndPos downto nStartPos do
        begin
            pd := DataList.Items[i];
            Dispose(pd);
            DataList.Delete(i);
...
begin
    ...
    begin
        ...
        else if pd^.StringKey > aKey then
        begin
            HighPos := MidPos - 1;
        end
        else
...
begin
    ...
    begin
        ...
        if pd^.StringKey = aKey then
        begin
            while MidPos <= DataList.Count - 1 do
            begin
                pd := DataList.Items[MidPos];
                if pd^.StringKey <> aKey then break;
                Inc(MidPos);
...
begin
    ...
    begin
        pd := DataList.Items[MidPos];
        if pd^.IntegerKey = aKey then
        begin
            Result := MidPos;
            exit;
        end else if pd^.IntegerKey > aKey then
        begin
            HighPos := MidPos - 1;
        end else
        begin
...
begin
    ...
    begin
        ...
        else if pd^.IntegerKey > aKey then
        begin
            HighPos := MidPos - 1;
        end
        else
...
constructor TStringKeyListClass.Create;
begin
    inherited Create;

end;

destructor TStringKeyListClass.Destroy;
begin

    inherited Destroy;
end;

function TStringKeyListClass.GetIndex(aIndex:integer):Pointer;
var
    pd              :PTStringKeyData;
begin
    result := nil;
...
constructor TIntegerKeyListClass.Create;
begin
    inherited Create;

end;

destructor TIntegerKeyListClass.Destroy;
begin

    inherited Destroy;
end;

function TIntegerKeyListClass.GetIndex(aIndex:integer):Pointer;
var
    pd              :PTIntegerKeyData;
begin
    result := nil;
...
function TStringKeyClass.getDataList: TList;
begin
  Result:=Self.DataList;
end;

function TStringKeyClass.SelectByIndex(aindex: Integer): Pointer;
var
//    i               :Integer;
    HighPos, LowPos, MidPos:Integer;
    pd              :PTStringKeyData;
