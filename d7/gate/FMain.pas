...
uses
  ...
  uPackets, AnsStringCls, DefType, WinSock;
...
type
  ...
  public
    ...
    function UpdateServer (CurTick : LongWord) : Boolean;
...
var
  ...
  StartTick, ElaspedSec : LongWord;
  BalanceSendTick : LongWord;

  RejectCharName : TStringList;

  ServerName : String;
  LimitUserCount : Integer;
  LimitPacketCount : integer;
  ...
  // 캐릭터 처음 생성할때 넣어줄것들...
  INI_CLOTHES_COAT_MAN : String;
  INI_CLOTHES_PANTS_MAN : String;
  INI_CLOTHES_COAT_WOMAN : String;
  INI_CLOTHES_PANTS_WOMAN : String;
  INI_WEAPON_SWORD : String;
  INI_WEAPON_KNIFE : String;
  INI_WEAPON_SPEAR : String;
  INI_WEAPON_AX : String;
  INI_ETC_01 : String;
...
begin
   ...
   if FileExists ('.\REMOTEIP.TXT') then begin
   ...
   if not FileExists ('.\GATE.INI') then begin
      ...
      iniFile.WriteString ('GATE_SERVER', 'SERVERNAME', Conv('창조'));
      iniFile.WriteInteger ('GATE_SERVER', 'LIMITUSERCOUNT', 0);
// add by minds 050908
      iniFile.WriteInteger('GATE_SERVER', 'LIMITPACKETCOUNT', 10);
//
   ...
   ServerName := iniFile.ReadString ('GATE_SERVER', 'SERVERNAME', Conv('창조'));
   LimitUserCount := iniFile.ReadInteger ('GATE_SERVER', 'LIMITUSERCOUNT', 0);
// add by minds 050908
   LimitPacketCount := iniFile.ReadInteger ('GATE_SERVER', 'LIMITPACKETCOUNT', 10);
//
   ...
   for i := 0 to nCount - 1 do begin
      New (pd);
      pd^.Name := iniFile.ReadString ('VILLAGE', 'NAME' + IntToStr (i), Conv('선비촌'));
   ...
   iniFile := TINIFile.Create ('.\CreateChar.INI');
   INI_CLOTHES_COAT_MAN := iniFile.ReadString ('CLOTHES', 'CLOTHES_COAT_MAN', 'COATMAN'); 
   INI_CLOTHES_PANTS_MAN := iniFile.ReadString ('CLOTHES', 'CLOTHES_PANTS_MAN', 'PANTSMAN');
   INI_CLOTHES_COAT_WOMAN := iniFile.ReadString ('CLOTHES', 'CLOTHES_COAT_WOMAN', 'COATWOMAN');
   INI_CLOTHES_PANTS_WOMAN := iniFile.ReadString ('CLOTHES', 'CLOTHES_PANTS_WOMAN', 'PANTSWOMAN');
   INI_WEAPON_SWORD := iniFile.ReadString ('WEAPON', 'WEAPON_SWORD', 'SWORD');
   INI_WEAPON_KNIFE := iniFile.ReadString ('WEAPON', 'WEAPON_KNIFE', 'KNIFE');
   INI_WEAPON_SPEAR := iniFile.ReadString ('WEAPON', 'WEAPON_SPEAR', 'SPEAR');
   INI_WEAPON_AX := iniFile.ReadString ('WEAPON', 'WEAPON_AX', 'AX');
   INI_ETC_01 := iniFile.ReadString ('ETC', 'ETC_01', 'ETC01');
   iniFile.Free;
...
function TfrmMain.UpdateServer (CurTick : LongWord) : Boolean;
var
   Packet : TPacketData;
begin
...
var
   nRead : Integer;
   buffer : array[0..SocketProcessBytes] of byte;
begin
   nRead := Socket.ReceiveBuf (buffer, SocketProcessBytes);
...
procedure TfrmMain.sckDBConnectConnect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
   DBSender := TPacketSender.Create ('DB', BufferSizeS2S, Socket, false, false);
   DBReceiver := TPacketReceiver.Create ('DB', BufferSizeS2S, false);
...
var
   nRead : Integer;
   buffer : array[0..SocketProcessBytes - 1] of byte;
begin
   nRead := Socket.ReceiveBuf (buffer, SocketProcessBytes);
   if nRead > 0 then begin
...
procedure TfrmMain.sckLoginConnectConnect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
   LoginSender := TPacketSender.Create ('Login', BufferSizeS2S, Socket, false, false);
   LoginReceiver := TPacketReceiver.Create ('Login', BufferSizeS2S, false);
...
var
   Size, OptLen : Integer;
begin
   GameSender := TPacketSender.Create ('Game', BufferSizeS2S, Socket, false, false);
   GameReceiver := TPacketReceiver.Create ('Game', BufferSizeS2S, false);
...
begin
   ...
   PaidSender := TPacketSender.Create ('PaidSender', BufferSizeS2S, Socket, false, false);
   PaidReceiver := TPacketReceiver.Create ('PaidReceiver', BufferSizeS2S, false);
...
var
   nRead : Integer;
   buffer : array [0..SocketProcessBytes - 1] of char;
begin
