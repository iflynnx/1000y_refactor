...
uses
   uConnect, uGateConnector, uPackets;
...
begin
   sckGate.Active := false;
   sckUser.Active := false;
...
begin
   ...
   if nIndex >= 0 then begin
      ...
      PacketSender := TPacketSender.Create ('Sender', 4096, Socket, true, true ); //true    false
...
begin
   ...
   Caption := 'Balance (' + IntToStr (ConnectorList.Count) + ')';
end;

procedure TfrmMain.sckGateAccept(Sender: TObject;
  Socket: TCustomWinSocket);
begin
   if GateConnectorList.CreateConnector (Socket) = true then begin
      AddInfo ('GateServer Connected at ' + Socket.RemoteAddress);
      exit;
   end;
   Socket.Close;
end;
...
procedure TfrmMain.sckGateClientDisconnect(Sender: TObject;
  Socket: TCustomWinSocket);
begin
   if GateConnectorList.DeleteConnector (Socket) = true then begin
      AddInfo ('GateServer DisConnected at ' + Socket.RemoteAddress);
      exit;
   end;
end;
...
begin
   iSize := Socket.ReceiveBuf (buffer, 1024);
   if iSize > 0 then begin
      GateConnectorList.AddReceiveData (Socket, @buffer, iSize);
   end;
end;
