...
type
    ...
    public
        ...
        procedure AddSendDataToBalance(aData: PChar; aCount: Integer; aMsg: Byte);    //2015.11.26 在水一方
...
begin
    ...
    GateSender := TPacketSender.Create('Gate', BufferSize_GATE_SEND, aSocket);
    GateReceiver := TPacketReceiver.Create('Gate', BufferSize_GATE_RECE);
...
//    i: Integer;
    Packet: TPacketData;
  //  buffer          :array[0..MAX_PACK_SIZE2 - 1] of byte;
//    nSend, nSize: Integer;
begin
    GateReceiver.Update;
...
begin
    ...
    begin
        ...
        begin
            ...
            if GateConnector.GateNo = i then
            begin
                boFind := true;
                break;
            end;
        end;
        if boFind = false then
        begin
            GateNo := i;
            break;
        end;
...
begin
    ...
    begin
        GateConnector := DataList.Items[i];
        if GateConnector.Socket = aSocket then
        begin
...
begin
    ...
    begin
        GateConnector := DataList.Items[i];
        if GateConnector.GateNo = aGateNo then
        begin
...
procedure TGateConnectorList.AddSendDataToBalance(aData: PChar; aCount: Integer; aMsg: Byte);  //2015.11.26 在水一方
var
    i: Integer;
    GateConnector: TGateConnector;
begin
    ...
    begin
        GateConnector := DataList.Items[i];
        GateConnector.AddSendServerData(0, aMsg, aData, aCount);
    end;
end;
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                GateConnector := DataList.Items[j];
                if GateConnector.Socket = pd^.Socket then
                begin
