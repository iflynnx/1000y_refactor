...
uses
  Windows, SysUtils, Classes, A2Img, deftype, clType, StrDb, AUtil32, Forms,
  uAnsTick;
...
const
   ETC_VIEW_ATZ: string = '.\ect\view.atz';
   MUN_ETC_ATZ: string = '.\ect\munetc.atz';
...
type
  ...
  public
    ...
    // object debug용
    procedure   FreeImageLib(const afName: string);
    procedure   SavePXPY(const afName: string);
    procedure   SetPX(const afName: string; apx: integer);
    procedure   SetPY(const afName: string; apy: integer);
   ...
   TAnimater = class
    ...
    public
      ...
      procedure ReloadAnimation(aFileName: string);
      function CheckMotion(aMotion: integer): Boolean;
   end;
   ...
   TEtcAtzClass = class
    ...
    public
      ...
      property ImageLib: TA2ImageLib read munetcAtzLib;
      property A2Image[value: integer]: TA2Image read GetEtcAtz; default;
   end;
...
var
   ...
   EtcViewClass : TEtcAtzClass;
   EtcServerClass : TEtcAtzClass;
...
begin
   ...
   MagicImageLib.LoadFromFile ('newmagic.atz');
...
// for Dynamic Object px,py setting용..
procedure   TAtzClass.FreeImageLib(const afName: string);
var
   i: integer;
begin
   for i := 0 to DataStringList.Count -1 do begin
      if CompareText(DataStringList[i], afName) = 0 then begin
         DataStringList.Delete(i);
         exit;
      end;
   end;
...
procedure   TAtzClass.SavePXPY(const afName: string);
var
   i: integer;
   Lib: TA2ImageLib;
begin
   for i := 0 to DataStringList.Count -1 do begin
      if CompareText(DataStringList[i], afName) = 0 then begin
         Lib := TA2ImageLib(DataStringList.Objects[i]);
         Lib.SaveToFileHeader('sprite\'+afName);
         exit;
      end;
   end;
end;

procedure   TAtzClass.SetPX(const afName: string; apx: integer);
var
   i, cnt: integer;
   ImageLib: TA2ImageLib;
begin
   for i := 0 to DataStringList.Count-1 do begin
      if CompareText(DataStringList[i], afName) = 0 then begin
         ...
         for cnt := 0 to ImageLib.Count-1 do
            ImageLib[cnt].px := ImageLib[cnt].px + apx;
         exit;
      end;
   end;
end;

procedure   TAtzClass.SetPY(const afName: string; apy: integer);
var
   i, cnt: integer;
   ImageLib: TA2ImageLib;
begin
   for i := 0 to DataStringList.Count-1 do begin
      if CompareText(DataStringList[i], afName) = 0 then begin
         ImageLib := TA2ImageLib(DataStringList.Objects[i]);
         for cnt := 0 to ImageLib.Count-1 do
            ImageLib[cnt].py := ImageLib[cnt].py + apy;
         exit;
      end;
   end;
end;
...
function TAtzClass.GetResCount: integer;
begin
...
function TAnimater.CheckMotion(aMotion: integer): Boolean;
var
   List: TList;
   pa: PTAniInfo;
   i: integer;
begin
   ...
   List := GetAnimationList(0);
   for i := 0 to List.Count-1 do begin
      pa := PTAniInfo(List[i]);
      if pa.Action = aMotion then exit;
   end;
...
begin
   ...
   for j := 1 to 1000 do begin
      ...
      with pa^ do begin
         ...
         if CompareText (Str, 'HIT10R') = 0 then Action := AM_HIT10_READY;
         if CompareText (str, 'HIT11') = 0  then Action := AM_HIT11;
         if CompareText (str, 'HIT11R') = 0  then Action := AM_HIT11_READY;

         if CompareText (str, 'SHIT1') = 0  then Action := AM_SHIT_WRESTLE;
         if CompareText (Str, 'SHIT1R') = 0 then Action := AM_SHIT_WRESTLE_READY;
         if CompareText (str, 'SHIT2') = 0  then Action := AM_SHIT_SWORD;
         if CompareText (Str, 'SHIT2R') = 0 then Action := AM_SHIT_SWORD_READY;
         if CompareText (str, 'SHIT3') = 0  then Action := AM_SHIT_BLADE;
         if CompareText (Str, 'SHIT3R') = 0 then Action := AM_SHIT_BLADE_READY;
         if CompareText (str, 'SHIT4') = 0  then Action := AM_SHIT_SPEAR;
         if CompareText (Str, 'SHIT4R') = 0 then Action := AM_SHIT_SPEAR_READY;
{ // 추가할것
...
procedure TAnimater.ReloadAnimation(aFileName: string);
var
   i, j: integer;
   List: TList;
begin
   for i := 0 to AniStringList.Count -1 do begin
      if aFileName = AniStringList[i] then begin
         List := TList (AnistringList.Objects [i]);
         for j := 0 to List.Count -1 do dispose (List[j]);
         List.Clear;
         LoadFromFile(aFileName, List);
         AnistringList.Objects[i] := List;
         exit;
      end;
   end;
end;
...
//                        TEtcAtzClass
////////////////////////////////////////////////////////////////////////////////
constructor TEtcAtzClass.Create(const filename: string);
begin
   munetcAtzLib := TA2ImageLib.Create;
   if not FileExists (filename) then exit;
   munetcAtzLib.LoadFromFile (filename);
end;
