...
uses FMain, FExchange, FLogOn, FBottom, FDepository, FAttrib, FHistory;
      ...
      SH_Server :
         begin
            ...
            if (i > 0) and (i <= CountMax) then begin
               ...
               FrmExchange.ExchangeLock := FALSE;
               Visible := FALSE;
               ChatMan.AddChat (Conv('数量已满或没有'), 9, 29);
               //FrmBottom.EdChat.SetFocus;
            end;
         end;
      SH_Depository :
...
begin
   case key of
      VK_RETURN: BtnOK.OnClick(Self);
      VK_ESCAPE: BtnCancel.OnClick(Self);
      VK_PRIOR:  btnMaxClick(Self);
      VK_NEXT:   EdCount.Text := '0';
   end;
end;
...
procedure TFrmQuantity.btnMaxClick(Sender: TObject);
begin
   //
