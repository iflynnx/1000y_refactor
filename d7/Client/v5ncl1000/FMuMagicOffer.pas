...
begin
    ...
    case PTSSShowGuildMagicWindow^.rWindow of
        WINDOW_GUILDMAGIC:
            begin
                with PTSSShowGuildMagicWindow^ do
                begin
                    {
...
begin
    ...
    A2ButtonOk.A2Up := EtcAtzClass.GetEtcAtz(97);
    A2ButtonOk.A2Down := EtcAtzClass.GetEtcAtz(98);
...
begin
    ...
    A2ILabelMagic.Transparent := true;
    Top := 0;
    Left := 0;
    SetImage;
end;
...
begin
    SetPostion;
    A2Edit2.SetFocus;
    A2LabelMsg1.Caption := ('请在门派武功名的后面输入框中输入门派武功名称');
    A2LabelMsg2.Caption := '';
...
begin
    ...
    begin
        ...
        Frmfbl.SocketAddData(sizeof(CGuildMagicData), @CGuildMagicData);
    end;
...
begin                                                                           // 傍拜加档
    A2LabelMsg1.Caption := ('攻击速度(最高99)+上身攻击之合100') + '(' + IntToStr(_StrToInt(A2EditSpeed.text) + _StrToInt(A2EditDamegeBody.Text)) + ')';
    A2LabelMsg2.Caption := '';
end;
...
begin                                                                           //磊技焊沥
    A2LabelMsg1.Caption := ('恢复(最高99)+躲闪之合是100') + '(' + IntToStr(_StrToInt(A2EditRecovery.text) + _StrToInt(A2EditAvoid.Text)) + ')';
    A2LabelMsg2.Caption := '';
end;
...
procedure TFrmMuMagicOffer.A2EditDamageHeadMouseDown(Sender: TObject;
    Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
    A2LabelMsg1.Caption := ('头部攻击+手臂攻击+腿部攻击+上身防御+头部防御 ');
    A2LabelMsg2.Caption := ('+手防+腿防之合228') + '(' +
...
procedure TFrmMuMagicOffer.A2EditOutPowerMouseDown(Sender: TObject;
    Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
    A2LabelMsg1.Caption := ('外功消耗+内功消耗+武功消耗+');
    A2LabelMsg2.Caption := ('活力消耗之合是80') + '(' +
