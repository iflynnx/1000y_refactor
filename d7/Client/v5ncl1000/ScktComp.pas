...
uses SysUtils, Windows, Messages, Classes, WinSock, SyncObjs,wininet;
...
type
  ...
  public
    ...
    function  WaitForData(Timeout: Longint): Boolean;   //防止断线过快
    function ReceiveLength: Integer;
    function ReceiveBuf(var Buffer; BufferSize: integer): Integer;
    function ReceiveText: string;
    function SendBuf(var Buffer; BufferSize: integer): Integer;
...
function GetInterNetURLText(lpcInterNetURL: PChar): string;
implementation
...
begin
hRequest :=  InternetOpenUrlA(hSession, lpcInterNetURL, nil, 0, INTERNET_FLAG_RELOAD,0);
...
function TCustomWinSocket.SendBuf(var Buffer; BufferSize: integer): Integer;
...
begin
  ...
  try
    repeat
//      if not Idle(60000) then Exit;
      result:= Send(FSocket, Buffer , BufferSize , 0);
      if  BufferSize = result then break;
...
procedure TCustomWinSocket.xDisconnect;
begin
  closesocket(FSocket);
  FConnected := False;
end;
function TCustomWinSocket.WaitForData(Timeout: Longint): Boolean;   //防止断线过快
var
  FDSet: TFDSet;
  TimeVal: TTimeVal;
begin
  ...
  begin
    ...
    FD_ZERO(FDSet);
    FD_SET(Fsocket, FDSet);
    //Result := select(0, @FDSet, nil, nil, @TimeVal) > 0; // 15.11.4 by nirendao
    Result := select(0, nil, @FDSet,  nil, @TimeVal) > 0;
  end;
end;
function TCustomWinSocket.ReceiveBuf(var Buffer; BufferSize: integer): Integer;
...
begin
  ...
  begin
  repeat
    if not WaitForData(60000) then exit;
    ...
    begin
      ...
      begin
        ...
        try {##Fix By Manuel Parma mparma@usa.net}
          ...
          on E: Exception do {##Fix By Manuel Parma mparma@usa.net}
          begin {##Fix By Manuel Parma mparma@usa.net}
            if not (E is EAbort) then {##Fix By Manuel Parma mparma@usa.net}
              raise;                  {##Fix By Manuel Parma mparma@usa.net}
...
begin
  ...
  FClientSocket := Asocket; {##Fix By Manuel Parma mparma@usa.net} {This fix avoid exception error in thread create handle }
