...
begin
  ...
  //  if not Assigned(DirectSoundCreate) then
    ...
    if DirectSoundCreate8(pg, FDirectSound8, nil) <> DS_OK then exit;
...
begin
    ...
    if FAOwner.DirectSound = nil then exit;
    if (FAOwner.DirectSound.CreateSoundBuffer(dsbd, dsbuffer, nil)) <> DS_OK then exit;
...
begin
    ...
    try
        ...
        NeedBuffer;
        if FAOwner.FDirectSound8 = nil then exit;
        Prepare;
        try
...
var
    dwResults       :array[0..conNUM_SFX - 1] of dword;
    i               :Byte;
