...
begin
    ...
    try
        ...
        begin
            if AuctionDBAdapter.Select(tempList.Strings[i], @tempData) <> DB_OK then Continue;
...
begin
    ...
    begin
       ...
       // if AuctionDBAdapter.Select(inttostr(pUserAucdata.rid), @fAuctionData) <> DB_OK then Continue;
...
begin
    ...
    aid := StrToInt(StringGrid_Auction.Rows[arow].Strings[1]);
//数据库中读
    if AuctionDBAdapter.Select(InttoStr(aid), @tempAuctionData) <> db_ok then
    begin
        ShowMessage('纪录不存在');
        exit;
    end;
...
begin
  //读最 新
    ...
    if AuctionDBAdapter.Select(Edit_Auction_ID.Text, @aAuctionData) = db_ok then
    begin
    ...
    begin
       //修改
        ...
        //保存
        if AuctionDBAdapter.Update(Edit_Auction_ID.Text, @tempAuctionData) = DB_OK then
        begin
            ShowMessage('保存成功');
        end else
        begin
...
begin
    ...
    if AuctionDBAdapter.Select(Edit_Auction__find_ID.Text, @aAuctionData) = DB_OK then
    begin
