...
function GetTimeDiff(time1, time2: TDateTime): INTEGER; //计算2个时间的时间差
...
var
  ...
  timeDiff: integer;
begin
  TimeStamp := DateTimeToTimeStamp(time1 - time2); // 算出相差值
  Dec(TimeStamp.Date, TTimeStamp(DateTimeToTimeStamp(0)).Date);
  timeDiff := (TimeStamp.Date * 24 * 60 * 60) + (TimeStamp.Time div 1000); //计算出秒
  Result := timeDiff;
end;
