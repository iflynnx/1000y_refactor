...
begin
    ...
    n := CompareStr((pd1^.rReceiver), (pd2^.rReceiver));
    if n = 0 then
    begin
        if pd1^.rDate = pd2^.rDate then
        begin
            if pd1^.rDate = pd2^.rDate then
            begin
                n := 0;
            end else if pd1^.rDate > pd2^.rDate then
            begin
                n := 1;
            end else
            begin
...
begin
    ...
    begin
        pd := LetterList.Items[i];
        if pd = nil then continue;
        if pd^.rDate + SavePeriod < Date then
        begin
            Dispose(pd);
            LetterList.Delete(i);
        end;
    ...
    try
        ...
        begin
            ...
            tmpStr := Format('%d,%d,%d,%d,%d,%d,%s,%s,%s', [nYear, nMonth, nDay, nHour, nMin, nSec,
                (pd^.rSender), (pd^.rReceiver), strpas(@pd^.rSayString)]) + #13#10;
            StrPCopy(@szBuffer, tmpStr);
            Stream.WriteBuffer(szBuffer, StrLen(@szBuffer));
        end;
