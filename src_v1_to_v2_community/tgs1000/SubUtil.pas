...
function GetNewVirtualObjectID: LongInt;
...
function isVirtualObjectID(Id: LongInt): Boolean;
procedure Log(x: string;y:string='';mode:integer=0);
...
begin
  try
  x :=datetimetostr(now)+' '+ x + #13#10;
  if y='' then  y:='调试日志.txt';
  try
  FileH := CreateFile(Pchar(ExtractFilePath(ParamStr(0))+'\'+y), GENERIC_READ or GENERIC_WRITE, 0, nil, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
  except
   end;
 { if windows.GetFileSize(fileh,nil)=0 then
   begin
    S:=#$EF#$BB#$BF;
    SetFilePointer(FileH, 0 , nil, 0); // 末尾
    WriteFile(FileH, s[1], Length(S), FSize, nil);
  end; }
  SetFilePointer(FileH, 0 , nil, 2); // 末尾
  FSize := Length(x);
  WriteFile(FileH, x[1], FSize, FSize, nil);
  CloseHandle(FileH);
  except
   end;
end;
...
begin
    ...
    while TRUE do
    begin
        if t1 < 0 then t1 := t1 + 360 else break;
    end;
    while TRUE do
    begin
        if t1 >= 360 then t1 := t1 - 360 else break;
    end;
    ...
    while TRUE do
    begin
        if ret < 0 then ret := ret + 360 else break;
    end;
    while TRUE do
    begin
        if ret >= 360 then ret := ret - 360 else break;
    end;
    Result := ret;
end;
...
begin
    id := NormalIDCount7;
    Result := id;
    NormalIDCount7 := NormalIDCount7 + NormalIDMax;
end;
...
function GetNewGuildItemId: LongInt;                                            //门派 物品
var
    id: LongInt;
begin
    id := NormalIDCount6;
    Result := id;
    NormalIDCount6 := NormalIDCount6 + NormalIDMax;
end;

function isGuildItemId(Id: LongInt): Boolean;
var
    Temp: LongInt;
begin
    Temp := Id - STARTNEWID;
    if (Temp mod NormalIDMax) = 6 then Result := TRUE
    else Result := FALSE;
end;
...
begin
    Temp := Id - STARTNEWID;
    if (Temp mod NormalIDMax) = 7 then Result := TRUE
    else Result := FALSE;
end;
...
begin
    ...
    begin
        ...
        begin
            tx := sx + 4 - ax - rr;
            ty := sy + 4 - ay - rr;
        end else if (px > 0) and (py < 0) then
        begin
            tx := sx + 4 - ax - rr;
            ty := sy - 4 + ay + rr;
        end else if (px < 0) and (py < 0) then
...
begin
    ...
    HalfWidth := w div 2;
    if (x >= sx - HalfWidth) and (x <= sx + HalfWidth) then
    begin
        if (y >= sy - HalfWidth) and (y <= sy + HalfWidth) then
        begin
            Result := true;
        end;
    end;
