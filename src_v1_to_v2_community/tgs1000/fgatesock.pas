...
uses
  ...
  ScktComp, adeftype, barutil, WinSock, JSocket ;
...
begin
   try
      ...
      while cnt > 0 do begin
         if cnt > MAX_PACK_SIZE2 then nRead := MAX_PACK_SIZE2
         else nRead := cnt;
...
var
   i : integer;
   cnt, ret : integer;
   buffer : array [0..MAX_PACK_SIZE2] of byte;
begin
   Result := FALSE;
   for i := 0 to 4-1 do begin
      if SSDataArr[i].rSocket = nil then continue;
      ...
      cnt := SSDataArr[i].rMyServersBigBufferClass.Get (MAX_PACK_SIZE2, @Buffer);
