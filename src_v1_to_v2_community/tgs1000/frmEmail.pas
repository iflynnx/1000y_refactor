...
uses
    ...
    Dialogs, StdCtrls, ExtCtrls, ComCtrls, Grids, deftype, uemail, uKeyClass, uEmaildata;
...
begin
    ...
    try
        templist.Text := EmailList.DBFileClass.SelectAllName;
        for i := 0 to templist.Count - 1 do
        begin
            if EmailList.DBFileClass.Select(templist.Strings[i], @tempdb) <> DB_OK then Continue;
...
begin
    ...
    fAllEmail.loaddb;
    ListBox1.Items.Text := fAllEmail.GetMenu;
end;
...
begin
    ...
    begin
     ...
     //   if EmailList.DBFileClass.Select(inttostr(pEmaildata.rid), @fEmailData) <> DB_OK then Continue;
        showPersonEmailData(i + 1, pEmaildata^);
    end;
    Button_change_Email.Enabled := true;
end;
 //邮件内容(NAME ID TITLE)
...
begin
    ...
    if EmailList.DBFileClass.Select(Edit_find_Email_ID.Text, @fEmailData) = DB_OK then
    begin
...
begin
    ...
    if EmailList.DBFileClass.Select(Edit_Email_ID.text, @aEmailData) <> db_ok then
    begin
    ...
    begin
        ...
        if EmailList.DBFileClass.Update(Edit_Email_ID.text, @aEmailData) = DB_OK then
        begin
            ShowMessage('保存成功');
        end else
        begin
...
begin
    ...
    aID := StrToInt(StringGrid_Email.Rows[arow].Strings[2]);
  //数据库中读

    if EmailList.DBFileClass.Select(InttoStr(aID), @tempEmailData) <> db_ok then
    begin
        ShowMessage('无纪录');
        exit;
    end;
...
function TUserEmailListClass.GetMenu: string;
var
    i: Integer;
    p: TUserEmailClass;
begin
...
procedure TFormEmail.ListBox1Click(Sender: TObject);
begin
    if ListBox1.ItemIndex < 0 then exit;
    Edit_Email_find_name.Text := ListBox1.Items.Strings[ListBox1.ItemIndex];
