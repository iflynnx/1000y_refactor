...
var
  ...
  cDefaultXmlFormat: TXmlFormatType = xfReadable;
...
{$ENDIF}
    ...
    function ReadAttributeColor(const AName: string; ADefault: TColor{$IFDEF D4UP} = clBlack{$ENDIF}): TColor; virtual;
...
{$ENDIF}
  ...
  private
    ...
    FFilePath: WideString; //by nirendao
    FDeCrypted: Boolean;
  ...
  public
    ...
    property DeCrypted: Boolean read FDeCrypted write FDeCrypted;
...
begin
  ...
  repeat
    ...
    if SkipQuotes then
    begin
      if InQuotes and (Ch = QuoteChar) then
      begin
        InQuotes := false;
      end
      else
    ...
    begin
      ...
      begin
        ...
        if xcChildValues in Options then
          if AnsiCompareText(Nodes[i].ValueAsString, ANode.Nodes[AIndex].ValueAsString) <> 0 then
          begin
            if assigned(MismatchNodes) then
              MismatchNodes.Add(Nodes[i]);
            ChildResult := False;
...
begin
  ...
  begin
    ...
    if (AnsiCompareText(ANode.Name, NodeName) = 0) and
      ...
      (AnsiCompareText(ANode.AttributeByName[AttribName], AttribValue) = 0) then
    begin
      Result := ANode;
      exit;
    end;
...
var
  i: integer;
begin
  //todo:  在nativexml.free时候以下代码引起错误.
  //原因为XMLNODE非引用传递造成
  for i := 0 to NodeCount - 1 do
  begin
...
function TXmlNode.ReadAttributeColor(const AName: string; ADefault: TColor): TColor;
var
  AIndex: integer;
begin          
  Result := ADefault;
  if length(AttributeByName[AName]) > 0 then
    Result := StrToInt(AttributeByName[AName]);
end;
...
begin
  ...
  if assigned(Document) and Document.AbortParsing then
    exit;
    ...
    begin
      ...
      begin
        try
          ...
          case ElementType of
            xeNormal, xeDeclaration, xeStyleSheet:
              begin
                ...
                // Is it a direct tag?
                IsDirect := False;
                if (ElementType = xeNormal) and (ALength > 0) and (AValue[ALength] = '/') then
                begin
                  dec(ALength);
                  IsDirect := True;
                end;
                ...
                repeat
                  ...
                  if S.Read(Ch, 1) <> 1 then
                    raise EFilerError.CreateFmt(sxeMissingCloseTag, [Self.FDocument.FilePath]);
                  ...
                  begin
                    ...
                    inc(AValuePos);
                    if AValuePos > AValueLength then
                    begin
                ...
                if (NodeCount > 0) and (Nodes[0].ElementType = xeCharData) then
                begin
          ...
          if assigned(Document) and not Document.AbortParsing then
          begin
...
begin
  // * 2009-07-16 赵松　
  //SetAttributeByName(AName, FromWidestring(AName));
...
begin
  ...
  begin
    ...
    if length(AttributeValue[i]) > 0 then
      ...
      begin
        ...
        begin
          ...
          if ThisNode.ElementType = xeCharData then
            AddLineFeed := False;
          if assigned(NextNode) and (NextNode.ElementType = xeCharData) then
            AddLineFeed := False;
          if AddLineFeed then
            WriteStringToStream(S, ALineFeed);
        end;
...
procedure TXmlNode.WriteAttributeBool(const AName: string; AValue,
  ADefault: boolean);
var
  Index: integer;
begin
...
begin
  ...
  if (FRootNodes.NodeCount > 0) and (FRootNodes[0].ElementType = xeDeclaration) then
    Result := UpperCase(FRootNodes[0].AttributeByName['encoding']);
end;
...
procedure TNativeXml.LoadFromFile(const FileName: WideString);
var
  S: TStream;
  s2: TMemoryStream;
  TmpStrings: TStrings;
...
begin
  ...
  begin
    ...
    try
      ...
      TmpString := TmpStrings.Text;
{$IFDEF Encry}
      TmpStrings.Text := UncrypStr(TmpString, FDeCryptString);

      TmpStrings.SaveToStream(s2);
{$ENDIF}
      //   S := TFileStream.Create('daaa', fmOpenRead or fmShareDenyWrite);
...
begin
  ...
  begin
    ...
    if NormalCount = 0 then
      raise EFilerError.Create(sxeNoRootElement + 'a' + ffilepath);
...
procedure TNativeXml.SaveToFile(const FileName: WideString);
var
  S: TStream;
begin
  if FileName = '' then
...
begin
  ANode := FRootNodes.NodeByElementType(xeComment);
  if not assigned(ANode) and (length(Value) > 0) then
  begin
...
destructor TsdCodecStream.Destroy;
begin
  // FStream.Free;
  inherited;
end;
