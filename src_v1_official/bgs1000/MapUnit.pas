...
type
   ...
   TMaper = Class
    ...
    public
     ...
     function   MapProc (Id: LongInt; Msg, x1, y1, x2, y2: word): Integer;
...
function TMaper.MapProc (Id: LongInt; Msg, x1, y1, x2, y2: Word): Integer;
begin
   Result := 0;
   ...
   case Msg of
      MM_MOVE :
         begin
            if MapUserArr[y1*FWidth + x1].id = id then MapUserArr[y1*FWidth + x1].id := 0;
            MapUserArr[y2*FWidth + x2].id := Id;
         end;
      MM_SHOW  :
         begin
            MapUserArr[y1*FWidth + x1].id := Id;
         end;
      MM_HIDE :
         begin
            if MapUserArr[y1*FWidth + x1].id = id then MapUserArr[y1*FWidth + x1].id := 0;
