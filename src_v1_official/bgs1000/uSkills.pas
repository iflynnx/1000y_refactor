...
type
   ...
   protected
      ...
      procedure Initial (aMonsterName: string);
...
uses
   svMain, SubUtil, aUtil32, svClass, uAIPath, FieldMsg,
...
procedure  TLifeObject.Initial (aMonsterName: string);
begin
...
begin
   ...
   if CopiedList <> nil then begin
      for i := 0 to CopiedList.Count - 1 do begin
         TLifeObject (CopiedList[i]).CopyBoss := nil;
         TLifeObject (CopiedList[i]).FBoAllowDelete := true;
      end;
      CopiedList.Free;
      CopiedList := nil;
   end;
...
begin
   ...
   lifepercent := CurLife * 100 div MaxLife;
...
begin
   ...
   if aBasicObject.BasicData.Feature.rRace = RACE_NPC then begin
      ...
      if (AttackMagic.rMagicType = MAGICTYPE_BOWING) and (boBowAvail = true) then begin
         ...
         if BowAvailTick + 100 * 20 < CurTick then begin
            boBowAvail := true;
            BowCount := 5;
         end;
      end;
