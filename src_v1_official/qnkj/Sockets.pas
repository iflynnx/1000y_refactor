...
procedure Register;
begin
  RegisterComponents('Notang', [TSockets]);
end;
...
//
  lin.l_onoff := 1;
  lin.l_linger := 0;
  setsockopt(FMSocket,SOL_SOCKET,SO_REUSEADDR,linx,sizeof(lin));       // 바인드 하기전에.. 리유즈 해야 함..
//
...
begin
  ...
  #13#10 + ErrMsg;
  if Assigned(FErrorOccurred) then
    FErrorOccurred(Self,Socket,error,sockfunc + ErrMsg)
  else
    begin
